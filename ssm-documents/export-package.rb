...
describe 'Test functionallity of stack-manager', type: :feature do
	...
	context 'Publish a message to SNS and check execution status' do
		...
		it 'should publish a message to SNS' do
			sns_client_publish = @sns_client.publish({
				subject: "Test Export Package",
                                message: "{ \"default\": \"{ 'task': 'export-package', 'stack_prefix': '#{@conf['stack_prefix']}, 'details': { 'component': '#{@component}', 'package_group': package_group, 'package_name': package_name, 'package_filter': package_filter }}\"}",
	...
	#context 'Check if Package is exported now ' do
